...
public class SPXCrossover extends Crossover {
	...
	public Solution doCrossover(double probability,
		...
		XReal[] P = new XReal[3];
		P[0] = new XReal(parent1);
		P[1] = new XReal(parent2);
		P[2] = new XReal(parent);
		...
		for (int i = 0; i < 3; i++) {
			x[i] = new Solution(parent1);
		}
		XReal[] X = new XReal[3];
		X[0] = new XReal(new Solution(parent1));
		X[1] = new XReal(new Solution(parent1));
		X[2] = new XReal(new Solution(parent1));
		...
		if (PseudoRandom.randDouble() <= probability) {//begin to SPX
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 100; i++) {//generate child
