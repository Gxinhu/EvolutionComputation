...
public class SolutionSet implements Serializable {
  ...
  */
	...
	public void sort(Comparator comparator) {
		if (comparator == null) {
			Configuration.getLogger_().severe("No criterium for comparing exist");
			return;
		} // if
	...
	public double[][] writeTransObjectivesToMatrix() {
		if (this.size() == 0) {
			return null;
		}
		double[][] objectives;
		...
		for (int i = 0; i < size(); i++) {
			for (int j = 0; j < get(0).getNumberOfObjectives(); j++) {
				objectives[i][j] = get(i).getTranslatedObjectives(j);
			}
		}
		return objectives;
