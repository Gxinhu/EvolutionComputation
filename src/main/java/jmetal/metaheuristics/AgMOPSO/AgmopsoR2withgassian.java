...
//	private Solution pbest_[];// _[];
    ...
    public AgmopsoR2withgassian(Problem problem) {
        super(problem);
    ...
    private void computeSpeed() throws JMException {
        ...
        for (int n = 0; n < this.population.size(); n++) {
            ...
            if (age[n] < max_age) {
                ...
                Solution pop = population.get(n);
                for (int var = 0; var < pop.getDecisionVariables().length; var++) {
                    pop.getDecisionVariables()[var].setValue(pop.getDecisionVariables()[var].getValue() + speed[n][var]);
                    if (pop.getDecisionVariables()[var].getValue() < problem.getLowerLimit(var)) {
                        pop.getDecisionVariables()[var].setValue(problem.getLowerLimit(var));
                        speed[n][var] = 0.0;//speed[n][var] * -1.0;
                    }
                    if (pop.getDecisionVariables()[var].getValue() > problem.getUpperLimit(var)) {
                        pop.getDecisionVariables()[var].setValue(problem.getUpperLimit(var));
                        speed[n][var] = 0.0;//speed[n][var] * -1.0;
                    }
                }
                ...
                updateReference(pop);
                // Update of solutions
                updateProblem(pop, n, speed[n]);
                //this.leaders_.add(particle);
//                this.archive.add(pop);
            } else {
                double[] temp = new double[population.get(n).getDecisionVariables().length];
                for(int ind=0;ind<population.get(n).getDecisionVariables().length;ind++){
                    temp[ind]=0;
                }
                double mean ;
                double sigma;
                ...
                for (int var = 0; var < population.get(n).getDecisionVariables().length; var++) {
                    ...
                    sigma=0.0;
                    speed[n][var] = 0;
                    mean = (gbest[var].getValue() - pbest[var].getValue()) / 2;
                    sigma = Math.abs(gbest[var].getValue() - pbest[var].getValue());
                    temp[var] = Math.sqrt(sigma) * r.nextGaussian() + mean;
                }
                Solution pop = population.get(n);
                for (int var = 0; var < pop.getDecisionVariables().length; var++) {
                    pop.getDecisionVariables()[var].setValue(temp[var]);
                    if (pop.getDecisionVariables()[var].getValue() < problem.getLowerLimit(var)) {
                        pop.getDecisionVariables()[var].setValue(problem.getLowerLimit(var));
                    }
                    if (pop.getDecisionVariables()[var].getValue() > problem.getUpperLimit(var)) {
