...
import jmetal.metaheuristics.Vapso.util.normalizationByNSGA3;
...
public class r2psoUseShiftedDistanceAndNormalDistance extends Algorithm {
...
//				calculateDistanceByPBId1(temp,functionValueMatrix);
	...
	private void calculateDistanceByPBId1(SolutionSet temp, RealMatrix functionValueMatrix) {
		double[] d1 = new double[temp.size()];
		for (int k = 0; k < temp.size(); k++) {
			double nl = 0.0;
			for (int m = 0; m < problem.getNumberOfObjectives(); m++) {
				d1[k] += temp.get(k).getTranslatedObjectives(m) * 1;
				nl += 1 * 1;
			}
			nl = Math.sqrt(nl);
