...
public class VagPso_changepbest extends Algorithm {
	...
	public VagPso_changepbest(Problem problem) {
		super(problem);
...
//		}
		...
		for (int i = 0; i < populationSize; i++) {
			theta[i] = k * (Math.toDegrees(Math.acos(angle[i])) + Math.toDegrees(cosineLambda[thetaId[i]]));
		}
