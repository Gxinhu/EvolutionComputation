...
public class psoForWFG extends Algorithm {
	...
	public psoForWFG(Problem problem) {
		super(problem);
...
//				}// IF
	...
	private void calculateCoefficientValuesModify(int index, double[] angle) {
		...
		vc = distance;
		double p1;
		double theta = Math.abs(cosineLambda[index] - Math.acos(angle[index])) * problem.getNumberOfObjectives();
		p1 = theta;
		//calculate F
		double f;
		f = 0.25;
	...
	private void offspringcreationbypso() throws JMException {
		SolutionSet temp = population.union(archive);
		RealMatrix tempMatrix = new Array2DRowRealMatrix(temp.writeObjectivesToMatrix());
		for (int i = 0; i < problem.getNumberOfObjectives(); i++) {
		...
		updatePopulationPso(pbestIndex, angle);
	}

	private void updatePopulationPso(int[] pbestIndex, double[] angle) throws JMException {
		int ran;
