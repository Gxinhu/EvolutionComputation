	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (evaluations < maxEvaluations) {
			...
			for (int ind = 0; ind < currentSolutionSet.size(); ind++) {
				...
				if (flag == -1) {
					newSolutionSet
							.add(new Solution(currentSolutionSet.get(ind)));
				}
				...
				if (flag == 1) { // The offSpring dominates
					...
					int compareResult = crowding.compare(individual,
							offSpring[0]);
					...
					if (!deleteMutant) {
						...
						newSolutionSet.add(new Solution(currentSolutionSet
								.get(ind)));
						archive.add(new Solution(offSpring[0]));
					}
				}
			...
			for (int j = 0; j < feedBack; j++) {
				if (archive.size() > j) {
					int r = PseudoRandom.randInt(0,
							currentSolutionSet.size() - 1);
