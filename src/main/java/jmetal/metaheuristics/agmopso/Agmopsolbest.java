...
public class Agmopsolbest extends Algorithm {
...
//	private Solution pbest_[];// _[];
	...
	public Agmopsolbest(Problem problem) {
		super(problem);
...
//		}
	...
	public double[] R2__(SolutionSet archive) {
		...
		for (int k = 0; k < lamdaVectors.length; k++) {
			for (int j = 0; j < archive.size(); j++) {
				TCH[k][j] = tch(lamdaVectors[k], archive.get(j));
			}
		}
		int[] index;
	...
	public double tch(double[] lambdaV, Solution p) {
		double[] temp;
	...
	int selectlbest(double[] R2indi, int i) {
		double max = 0;
		int maxindex = 0;
		for (int k = 0; k < this.neighborhood_[i].length; k++) {
			if (R2indi[neighborhood_[i][k]] > max) {
				max = R2indi[neighborhood_[i][k]];
				maxindex = k;
			}
	...
	private double[][] computeSpeed() throws JMException {
		...
		int l2;
		double[] R2indi;
		R2indi = new double[populationSize];
		R2indi = this.R2__(this.leader_ind);
		int lbestindex = 0;
...
//            lbest = leader_ind.get(p.get(0)).getDecisionVariables();
			lbestindex = selectlbest(R2indi, n);
			lbest = leader_ind.get(neighborhood_[n][lbestindex]).getDecisionVariables();
			;
