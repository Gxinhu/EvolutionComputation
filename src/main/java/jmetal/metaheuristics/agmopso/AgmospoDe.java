...
class AgmopsoDe extends Algorithm {
...
//	private Solution pbest_[];// _[];
	...
	public AgmopsoDe(Problem problem, QualityIndicator indicator, int i, boolean save) {
		super(problem);
	 ...
	 */
	public AgmopsoDe(Problem problem) {
		super(problem);
	}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (evelations < max_evelations) {
			...
			for (int i = 0; i < cpopulation.size(); i++) {
				Solution[] particle2 = new Solution[4];
				int ran;
				particle2[0] = cpopulation.get(i);
				ran = PseudoRandom.randInt(0, cpopulation.size() - 1);
				particle2[1] = cpopulation.get(ran);
				ran = PseudoRandom.randInt(0, cpopulation.size() - 1);
				particle2[2] = cpopulation.get(ran);
				double mindistance=distance_.distanceBetweenSolutions(particle2[0], particle2[2]);
				double mindistance2=distance_.distanceBetweenSolutions(particle2[1], particle2[2]);
				if(mindistance>mindistance2) {
					particle2[0]=particle2[1];
				}
				ran = PseudoRandom.randInt(0, cpopulation.size() - 1);
				particle2[3] = cpopulation.get(ran);
				ran = PseudoRandom.randInt(0, cpopulation.size() - 1);
				particle2[1] = cpopulation.get(ran);
				mindistance=distance_.distanceBetweenSolutions(particle2[3], particle2[2]);
				mindistance2=distance_.distanceBetweenSolutions(particle2[1], particle2[2]);
				if(mindistance<mindistance2) {
					particle2[1]=particle2[3];
				}
				Solution offSpring = (Solution) crossoverOperator.execute(new Object[] {
						particle2[2], particle2});
...
//		写入csv文件,随着迭代次数的指标变化
		savetofile savetofile = new savetofile(problem, "out/AgmopsoDE1/IGD/" + problem.getName(), runtimes, realtimeIGD);
		savetofile.save();
		savetofile = new savetofile(problem, "out/AgmopsoDE1/GD/" + problem.getName(), runtimes, realtimeGD);
		savetofile.save();
		savetofile = new savetofile(problem, "out/AgmopsoDE1/Spread/" + problem.getName(), runtimes, realtimeSpeard);
		savetofile.save();
		return archive;
	}
