...
public class DistanceShifted {
	...
	public double distanceBetweenObjectivesShiftedTranslated(Solution solutionI, Solution solutionJ) {
		...
		for (int nObj = 0; nObj < solutionI.getNumberOfObjectives(); nObj++) {
			diff = 0;
			if (solutionI.getTranslatedObjectives(nObj) < solutionJ.getTranslatedObjectives(nObj)) {
				diff = solutionI.getTranslatedObjectives(nObj) - solutionJ.getTranslatedObjectives(nObj);
			}
			distance += diff * diff;
		} // for
