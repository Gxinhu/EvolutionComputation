...
public abstract class Experiment {
    ...
    public Experiment() {
        ...
        indicatorMinimize_.put("Space", true);
        ...
        indicatorMinimize_.put("IGDPLUS", true);
                ...
                for (int problemIndex = 0; problemIndex < problemList_.length; problemIndex++) {
                    ...
                    for (String anIndicatorList_ : indicatorList_) {
                        ...
                        for (int numRun = 0; numRun < independentRuns_; numRun++) {
                            ...
                            if (anIndicatorList_.equals("HV2")) {
                            ...
                            if (anIndicatorList_.equals("IGD")) {
                                CEC_IGD indicators = new CEC_IGD();
                                double[][] solutionFront =
                                        indicators.utils_.readFront(solutionFrontFile);
                            ...
                            if (anIndicatorList_.equalsIgnoreCase("IGDPLUS")) {
                                InvertedGenerationalDistancePlus indicators = new InvertedGenerationalDistancePlus();
                                double[][] solutionFront =
                                        indicators.utils_.readFront(solutionFrontFile);


                                value = indicators.invertedGenerationalDistancePlus(solutionFront, indicator.getTrueParetoFront());

                                qualityIndicatorFile = qualityIndicatorFile + "/IGDPLUS";
                            }
                            ...
                            if (anIndicatorList_.equals("Space")) {
...
//			if (ifTest == true)
//				printMeanSymbols(latexFile, i, mean,testSymbols);
//			else 
//				printMeanStdDev(latexFile, i, mean);//
...
//			printMedianIQRSymbols(latexFile, i, median, iqr,testSymbols);	
    ...
    void printHeaderLatexCommands(String fileName) throws IOException {
        ...
        os.write("\\documentclass[a4paper]{article}" + "\n");
    ...
    void printMeanStdDev(String fileName, int indicator, double[][][] mean, double[][][] stdDev) throws IOException {
        ...
        for (int cyc = 0; cyc < cycles; cyc++) {
            ...
            for (int i = 0; i < problemList_.length; i++) {
                ...
                os.write(problemList_[i].replace("_", "\\_") + " " + noOfObjectives_ + 'M' + " & ");
                if (cyc < cycles - 1) {
            ...
            for (int i = 0; i < problemList_.length; i++) {
                ...
                if (cyc < cycles - 1) {
                    for (int j = cyc * numberofalg; j < (cyc + 1) * numberofalg; j++) {
                        ...
                        if (j < (cyc + 1) * numberofalg - 1) {
                            m = String.format(Locale.ENGLISH, "%.2e", mean[indicator][i][j]);
                            os.write("$" + m + "$ & ");
                        } else {
                            m = String.format(Locale.ENGLISH, "%.2e", mean[indicator][i][j]);
                            os.write("$" + m + "$ \\\\" + "\n");
                ...
                if (cyc == cycles - 1) {
                    ...
                    m = String.format(Locale.ENGLISH, "%.2e", mean[indicator][i][algorithmNameList_.length - 1]);
                    os.write("$" + m + "$ \\\\" + "\n");
                }
            } // for
            os.write("\\hline" + "\n");
