...
public class wfgHvPlatEMO {
    ...
    public double calculatewfghv() throws IOException {
        ...
        if (problemNames.contains("MaF")) {
            ...
            String fullName = StrUtil.format("{}-{}d", problemNames, number);
            JSONArray points = maxPoint.getJSONArray(fullName);
            IntStream.range(0, number).forEach(i -> referencePoint1.setObjective(i, points.getDouble(i)));
        } else if (problemNames != null) {
