...
//   public CaptureConvergence(String problem, String algName) {
//		/**
...
//		expName_ ="PaRPEAStudyConvergence";		
	...
	public void runCaptureConvergence(int FEs, SolutionSet solutionSet) {
		if (FEs == 0 || FEs % gap_ == 0) {
			...
			// convert solutionSet to non-dominated solution set 
			NonDominatedSolutionList nonDominatedSolutionSet = new NonDominatedSolutionList();
			for (int i = 0; i < solutionSet.size(); i++) {
				nonDominatedSolutionSet.add(solutionSet.get(i));
			}
			...
			resetFile(path);
			nonDominatedSolutionSet.printFeasibleFUN(path);
			times_++;
		}
	}
