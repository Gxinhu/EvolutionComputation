...
public class proportionalclone extends Clone {
	...
	public Object execute(Object parent) throws JMException {
		...
		for (k = 0; k < parents.size(); k++) {
			if (parents.get(k).getCrowdingDistance() != Double.POSITIVE_INFINITY) {//k is the first not the boundery solution
		...
		for (k = 0; k < parents.size(); k++) {
			...
			if (sum_distance == 0) {//all individual are to one point
