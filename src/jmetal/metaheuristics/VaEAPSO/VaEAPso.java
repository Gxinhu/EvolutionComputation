package jmetal.metaheuristics.VaEAPSO;

import jmetal.core.*;
import jmetal.metaheuristics.r2pso.util.ShiftedEuclideanDistanceAssigment;
import jmetal.metaheuristics.vamaea.Niching;
...
public class VaEAPso extends Algorithm {
	...
	public VaEAPso(Problem problem) {
		super(problem);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		alpha_ = Math.PI / 2 / (populationSize + 1);
		iteration = 0;
...
//			++iteration;
	...
	private void updatedAchieveByVAEA() {
		SolutionSet union = ((SolutionSet) archive).union(offspringPopulation);
		archive.clear();
		new Niching(archive, union, true, populationSize, alpha_).execute();
	}
	...
	private void updatedAchieveByR2() {
		...
		for (int k = 0; k < offspringPopulation.size(); k++) {
			archive.add(new Solution(offspringPopulation.get(k)));
		}
...
//		double theta=0.2-Math.acos(angle[index]);
		double theta = angle[index] - Math.PI / 4;
//		theta=0.2;
		p1 = theta;
		//calculate F
		double f;
