...
public class SDE_PSO_Angle_changePbest_DTLZBetter extends Algorithm {
	...
	public SDE_PSO_Angle_changePbest_DTLZBetter(Problem problem) {
		super(problem);
...
//				}// IF
	 ...
	 * @param angleArchive
	 */
	private void calculateCoefficientValuesModify(int index, double[] angle, double[] angleArchive) {
		//calculate Vc
		double vc;
		double distance = 0;
...
//		double theta=0.2-Math.acos(angle[index]);
		double theta = (Math.PI / 4 - Math.acos(angleArchive[index]));
		p1 = theta;
		//calculate F
		double f;
		f = 0.25;
	...
	private void offspringcreationbypso() throws JMException {
		...
		for (int i = 0; i < populationSize; i++) {
			theta[i] = k * problem.getNumberOfObjectives() * (Math.toDegrees(Math.acos(angle[i])));
		}
		double fitnesse;
		for (int i = 0; i < this.populationSize; i++) {
			best_ind = -1;
		...
		updatePopulationPso(pbestIndex, angle, angleArchive, thetaId);
	}

	private void updatePopulationPso(int[] pbestIndex, double[] angle, double[] angleArchive, int[] thetaId) throws JMException {
		int ran;
		...
		for (int i = 0; i < populationSize; i++) {
			calculateCoefficientValuesModify(i, angle, angleArchive);
