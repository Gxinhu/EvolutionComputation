...
public class MOEAD_STM extends Algorithm {
	...
	public void selection() {
		...
		for (int i = 0; i < populationSize_; i++) {
			for (int j = 0; j < union_.size(); j++)
				subpPref[i][j] = j;
			moeadUtils.QuickSort(subpMatrix[i], subpPref[i], 0, union_.size() - 1);
		}
		for (int i = 0; i < union_.size(); i++) {
			for (int j = 0; j < populationSize_; j++)
				solPref[i][j] = j;
			moeadUtils.QuickSort(solMatrix[i], solPref[i], 0, populationSize_ - 1);
		}
	...
	public void initUniformWeight() {
		...
		dataFileName = "W" + problem_.getNumberOfObjectives() + "d" + populationSize_ + ".dat";

//		dataFileName = "pW" + problem_.getNumberOfObjectives() + "d" + problem_.getName() + "_" + referenceIdx_ + ".dat";
