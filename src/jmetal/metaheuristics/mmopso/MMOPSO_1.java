...
public class MMOPSO_1 extends Algorithm {

	public MMOPSO_1(Problem problem) {
		//this.run = r;
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (evelations < max_evelations) {
			...
			for (int i = 0; i < population.size(); i++) {
				...
				// update pbest based on decomposition
				if (this.fitnessFunction(pbest_[i], this.lamdaVectors[i]) > this
						.fitnessFunction(population.get(i),
								this.lamdaVectors[i])) // the new particle is
				{
					pbest_[i] = new Solution(population.get(i));
				}
			}
	...
	private double[][] computeSpeed(double pro) throws JMException {
		...
		for (int n = 0; n < this.population.size(); n++) {
			...
			for (int var = 0; var < particle.length; var++) {
				...
				speed[n][var] = (W * velocity[n][var]) + C1 * r1
						* (pbest[var].getValue() - particle[var].getValue())
