...
public class MMOPSO_II extends Algorithm {

	public MMOPSO_II(Problem problem) {
		//this.run = r;
	...
	//int[] leader_ind;
	Solution[] leaders_archive;
	double[] leader_values;
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		leaders_archive = new Solution[populationSize];
		...
		leader_values = new double[populationSize];
		initUniformWeight();
		...
		while (evelations < max_evelations) {
			...
			for (int i = 0; i < temppopulation.size(); i++) {
				flag = archive.add(temppopulation.get(i));
				if (flag == true) {
					find_newleader(temppopulation.get(i));
				}
			}
	...
	public void find_leader() {
		...
		for (int i = 0; i < this.populationSize; i++) {
			...
			leader_values[i] = minFit;
			leaders_archive[i] = archive.get(best_ind);
		}
	}

	public void find_newleader(Solution new_one) {
		double fitnesse;
		double fitnesse12;
		for (int i = 0; i < this.populationSize; i++) {
			fitnesse = this.fitnessFunction(new_one, this.lamdaVectors[i]);
			fitnesse12 = this.fitnessFunction(leaders_archive[i], this.lamdaVectors[i]);
			if (fitnesse < fitnesse12) {
				leader_values[i] = fitnesse;
				leaders_archive[i] = new_one;
			}
		}
	}
	...
	public void evaluatePopulation(double[][] speed) throws JMException {
		...
		for (int i = 0; i < this.populationSize; i++) {
			...
			flag = archive.add(particle);
			if (flag == true) {
				find_newleader(particle);
			}
		}
	...
	private double[][] computeSpeed(double pro) throws JMException {
		...
		for (int n = 0; n < this.population.size(); n++) {
			...
			if (rand < pro) {
				...
				//l2 = leader_ind[n];
				//l2 = leader_ind[n];
				gbest = leaders_archive[n].getDecisionVariables();
			} else {
	...
	double fitnessFunction_ori(Solution individual, double[] lamda) {
