...
public class SMSEMOARandom extends Algorithm {
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (evaluations < maxEvaluations) {
			...
			for (selected = 0; selected < N_O; selected++) {
				...
				if (!found && (rnd <= contribution_[selected])) {
					if ("DE".equals(getOffspring[selected].id())) {
						Solution[] individual = (Solution[]) selectionOperator
								.execute(population);
						offSpring = getOffspring[selected].getOffspring(
								parents, individual[0]);
						// contrDE++;
					} else if ("SBXCrossover".equals(getOffspring[selected]
							.id())) {
						offSpring = getOffspring[selected]
								.getOffspring(parents);
						// contrSBX++;
					} else if ("PolynomialMutation"
							.equals(getOffspring[selected].id())) {
						offSpring = ((PolynomialMutationOffspring) getOffspring[selected])
								.getOffspring(parents[0]);
						// contrPol++;
					} else {
						System.out
								.println("Error in sMOCellAdaptive. Operator "
			...
			/*
			 * // crossover Solution[] offSpring = (Solution[])
			 * crossoverOperator.execute(parents);
			 *
			 * // mutation mutationOperator.execute(offSpring[0]);
			 */
			// evaluation
			...
			SolutionSet lastFront = ranking.getSubfront(ranking
					.getNumberOfSubfronts() - 1);
			if (lastFront.size() > 1) {
				...
				// front
				double[] maximumValues = utils_.getMaximumValues(
						union.writeObjectivesToMatrix(), numberOfObjectives);
				double[] minimumValues = utils_.getMinimumValues(
						union.writeObjectivesToMatrix(), numberOfObjectives);
				// STEP 2. Get the normalized front
				double[][] normalizedFront = utils_.getNormalizedFront(
						frontValues, maximumValues, minimumValues);
	...
	private double[] hvContributions(double[][] front) {
		...
		double totalVolume = hv_.calculateHypervolume(totalFront,
		...
		for (int i = 0; i < front.length; i++) {
			...
			// Zitzler code)
			double hv = hv_.calculateHypervolume(frontSubset,
